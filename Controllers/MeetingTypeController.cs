...
namespace MOM_Project.Controllers
{
    ...
    {
        // Static list to store meeting types (in real app, use database)
        private static List<MeetingTypeModel> _meetingTypes = new List<MeetingTypeModel>
        {
        ...
        [HttpPost]
        public IActionResult saveMeetingType(MeetingTypeModel meetingTypeModel)
        {
            if (ModelState.IsValid)
            {
                // Generate new MeetingTypeID
                var newId = _meetingTypes.Any() ? _meetingTypes.Max(m => m.MeetingTypeID) + 1 : 1;

                // Set created and modified dates
                meetingTypeModel.MeetingTypeID = newId;
                meetingTypeModel.Created = DateTime.Now;
                meetingTypeModel.Modified = DateTime.Now;

                // Add to the list
                _meetingTypes.Add(meetingTypeModel);

                // Redirect to list view
                return RedirectToAction("MeetingTypeList");
            }
            return View("MeetingTypeAddEdit", meetingTypeModel);
        }
    }
}