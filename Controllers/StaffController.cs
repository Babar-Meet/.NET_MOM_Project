...
namespace MOM_Project.Controllers
{
    ...
    {
        // Static list to store staff (in real app, use database)
        private static List<StaffModel> _staffList = new List<StaffModel>
        {
            new StaffModel {
                StaffID = 1,
        ...
        [HttpPost]
        public IActionResult saveStaff(StaffModel staffModel)
        {
            if (ModelState.IsValid)
            {
                // Generate new StaffID
                var newId = _staffList.Any() ? _staffList.Max(m => m.StaffID) + 1 : 1;

                // Set created and modified dates
                staffModel.StaffID = newId;
                staffModel.Created = DateTime.Now;
                staffModel.Modified = DateTime.Now;

                // Add to the list
                _staffList.Add(staffModel);

                // Redirect to list view
                return RedirectToAction("StaffList");
            }
            return View("StaffAddEdit", staffModel);
        }
    }
}