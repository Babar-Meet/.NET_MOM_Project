...
namespace MOM_Project.Controllers
{
    ...
    {
        // Static list to store meeting venues (in real app, use database)
        private static List<MeetingVenueModel> _meetingVenues = new List<MeetingVenueModel>
        {
        ...
        {
            ViewBag.MeetingVenues = _meetingVenues;
            return View();
        }
        ...
        [HttpPost]
        public IActionResult saveMeetingVenue(MeetingVenueModel meetingVenueModel)
        {
            if (ModelState.IsValid)
            {
                // Generate new MeetingVenueID
                var newId = _meetingVenues.Any() ? _meetingVenues.Max(m => m.MeetingVenueID) + 1 : 1;

                // Set created and modified dates
                meetingVenueModel.MeetingVenueID = newId;
                meetingVenueModel.Created = DateTime.Now;
                meetingVenueModel.Modified = DateTime.Now;

                // Add to the list
                _meetingVenues.Add(meetingVenueModel);

                // Redirect to list view
                return RedirectToAction("MeetingVenueList");
            }
            return View("MeetingVenueAddEdit", meetingVenueModel);
        }
    }
}